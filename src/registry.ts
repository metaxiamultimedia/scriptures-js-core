...
import type { Scheme } from './versification/schemes.js';
...
export interface ScriptureSource {
  ...
  /**
   * Versification scheme this edition numbers by (e.g. 'MT', 'LXX', 'English').
   * Self-declared here so a new edition aligns via `getParallelVerses`/`mapVerse`
   * with NO core change — `schemeFor()` reads this first, falling back to the
   * legacy hardcoded `EDITION_SCHEME` map only for editions that don't declare it.
   */
